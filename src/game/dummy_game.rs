...
impl DummyGame {
    ...
    pub fn new(title: &str) -> DummyGame {
        ...
        ms.push(drawables::StaticMesh::load_obj(&mut display, "./res/teapot.obj"));
                ...
                glutin::Event::WindowEvent { event, .. } => match event {
                    ...
                    glutin::WindowEvent::KeyboardInput { input, .. } => {
                        match input.virtual_keycode {
                            ...
                            Some(glutin::VirtualKeyCode::W) => if input.state == Pressed {w=Keystate::Pressed} else {w=Keystate::Released},
                            Some(glutin::VirtualKeyCode::S) => if input.state == Pressed {s=Keystate::Pressed} else {s=Keystate::Released},
                            Some(glutin::VirtualKeyCode::A) => if input.state == Pressed {a=Keystate::Pressed} else {a=Keystate::Released},
                            Some(glutin::VirtualKeyCode::D) => if input.state == Pressed {d=Keystate::Pressed} else {d=Keystate::Released},
                            Some(glutin::VirtualKeyCode::X) => if input.state == Pressed {x=Keystate::Pressed} else {x=Keystate::Released},
                            Some(glutin::VirtualKeyCode::Y) => if input.state == Pressed {y=Keystate::Pressed} else {y=Keystate::Released},
                            _ => (),
                        }
                    },
                    _ => (),
    ...
    fn render(&self) {
        ...
        for mesh in &self.meshes{
            ...
            target.draw( (&mesh.vertices, &mesh.normals), &mesh.indices, &self.renderer,
